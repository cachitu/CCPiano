...
public protocol CCMidiManagerProtocol: class {
    func didPlayNoteOn(_ channel:UInt32, noteNum:UInt32, velocity:UInt32)
    func didPlayNoteOff(_ channel:UInt32, noteNum:UInt32)
}

public class CCMIDIManager : NSObject {
    
    public var delegate: CCMidiManagerProtocol?
    ...
    public func initMIDI(midiNotifier: MIDINotifyBlock? = nil, reader: MIDIReadBlock? = nil) {
        ...
        os_log("initializing MIDI", log: CCMIDIManager.midiLog, type: .debug)
        ...
        if status == noErr {
            os_log("created MIDI client %d", log: CCMIDIManager.midiLog, type: .debug, midiClient)
        } else {
            os_log("error creating MIDI client %@", log: CCMIDIManager.midiLog, type: .error, status)
            checkError(status)
        }
        ...
        if status == noErr {
            ...
            if status == noErr {
                os_log("created input port %d", log: CCMIDIManager.midiLog, type: .debug, inputPort)
            } else {
                os_log("error creating input port %@", log: CCMIDIManager.midiLog, type: .error, status)
                checkError(status)
            }
            ...
            if status == noErr {
                os_log("created output port %d", log: CCMIDIManager.midiLog, type: .error, status)
                checkError(status)
            }
            ...
            //                                                    readBlock)
             ...
             if status == noErr {
                os_log("created virtual destination %d", log: CCMIDIManager.midiLog, type: .debug, virtualDestinationEndpointRef)
            } else {
                os_log("error creating virtual destination %@", log: CCMIDIManager.midiLog, type: .error, status)
                checkError(status)
            }
            ...
            if status == noErr {
                os_log("created virtual source %d", log: CCMIDIManager.midiLog, type: .debug, virtualSourceEndpointRef)
            } else {
                os_log("error creating virtual source %@", log: CCMIDIManager.midiLog, type: .error, status)
                checkError(status)
            }
  ...
  public func stopMidi() {
    ...
    MIDIPortDispose(self.midiInputPortref)
    ...
    func handle(_ packet:MIDIPacket) {
        ...
        case 0x80:
            ...
            DispatchQueue.main.async {
                self.delegate?.didPlayNoteOff(UInt32(channel), noteNum: UInt32(d1))
                //self.playNoteOff(UInt32(channel), noteNum: UInt32(d1))
            }
        ...
        case 0x90:
            ...
            // forward to sampler
            DispatchQueue.main.async {
                self.delegate?.didPlayNoteOn(UInt32(channel), noteNum:UInt32(d1), velocity: UInt32(d2))
                //self.playNoteOn(UInt32(channel), noteNum:UInt32(d1), velocity: UInt32(d2))
            }
    ...
    func showMIDIObjectType(_ ot: MIDIObjectType) {
        switch ot {
        case .other:
            os_log("midiObjectType: Other", log: CCMIDIManager.midiLog, type: .debug)
            break
            
        case .device:
            os_log("midiObjectType: Device", log: CCMIDIManager.midiLog, type: .debug)
            break
            
        case .entity:
            os_log("midiObjectType: Entity", log: CCMIDIManager.midiLog, type: .debug)
            break
            
        case .source:
            os_log("midiObjectType: Source", log: CCMIDIManager.midiLog, type: .debug)
            break
            
        case .destination:
            os_log("midiObjectType: Destination", log: CCMIDIManager.midiLog, type: .debug)
            break
            
        case .externalDevice:
            os_log("midiObjectType: ExternalDevice", log: CCMIDIManager.midiLog, type: .debug)
            break
            
        case .externalEntity:
            print("midiObjectType: ExternalEntity")
            os_log("midiObjectType: ExternalEntity", log: CCMIDIManager.midiLog, type: .debug)
            break
            
        case .externalSource:
            os_log("midiObjectType: ExternalSource", log: CCMIDIManager.midiLog, type: .debug)
            break
            
        case .externalDestination:
            os_log("midiObjectType: ExternalDestination", log: CCMIDIManager.midiLog, type: .debug)
            break
        }
    ...
    func connectSourcesToInputPort() {
        ...
        for srcIndex in 0 ..< sourceCount {
            ...
            if status == noErr {
                os_log("yay connected endpoint to inputPort", log: CCMIDIManager.midiLog, type: .debug)
            } else {
                print("oh crap!")
                checkError(status)
            }
    ...
    func disconnectSourceFromInputPort(_ sourceMidiEndPoint:MIDIEndpointRef) -> OSStatus {
        ...
        if status == noErr {
            ...
            os_log("could not disconnect inputPort %@ endpoint %@ status %@", log: CCMIDIManager.midiLog, type: .error, inputPort,sourceMidiEndPoint,status )
            checkError(status)
        }
        return status
    }
    ...
    func playWithMusicPlayer() {
        ...
        if let sequence = self.musicSequence {
            ...
            os_log("could not create sequence and play it", log: CCMIDIManager.midiLog, type: .error)
        }
    }
    ...
    internal func createMusicPlayer(musicSequence:MusicSequence) -> MusicPlayer? {
        ...
        if status != noErr {
            os_log("error creating music player %@", log: CCMIDIManager.midiLog, type: .error, status)
            checkError(status)
        }
        ...
        if let player = musicPlayer {
            ...
            if status != noErr {
                os_log("error setting sequence %@", log: CCMIDIManager.midiLog, type: .error, status)
                checkError(status)
            }
            
            status = MusicPlayerPreroll(player)
            if status != noErr {
                os_log("error prerolling music player %@", log: CCMIDIManager.midiLog, type: .error, status)
                checkError(status)
            }
            
            return player
        } else {
            os_log("music player is nil", log: CCMIDIManager.midiLog, type: .error)
            return nil
        }
    }
    ...
    internal func playMusicPlayer() {
        ...
        if let player = self.musicPlayer {
            ...
            if playing != false {
                os_log("music player is playing. stopping", log: CCMIDIManager.midiLog, type: .debug)

                status = MusicPlayerStop(player)
                if status != noErr {
                    os_log("error stopping %@", log: CCMIDIManager.midiLog, type: .error, status)
                    checkError(status)
                    return
                }
            } else {
                os_log("music player is not playing", log: CCMIDIManager.midiLog, type: .debug)
            }
            
            status = MusicPlayerSetTime(player, 0)
            if status != noErr {
                os_log("error setting time %@", log: CCMIDIManager.midiLog, type: .error, status)
                checkError(status)
                return
            }
            
            os_log("starting to play", log: CCMIDIManager.midiLog, type: .debug)

            status = MusicPlayerStart(player)
            if status != noErr {
                os_log("error starting %@", log: CCMIDIManager.midiLog, type: .error, status)
                checkError(status)
                return
            }
        }
    ...
    func createMIDIFile(sequence:MusicSequence, filename:String, ext:String)  {
        ...
        if let fileURL = NSURL(fileURLWithPath: documentsDirectory).appendingPathComponent("\(filename).\(ext)") {
            ...
            os_log("creating midi file at %@", log: CCMIDIManager.midiLog, type: .debug, fileURL.absoluteString)
    ...
    func determineTimeResolution(musicSequence:MusicSequence) -> UInt32 {
        ...
        if let tempoTrack = track {
            ...
            os_log("error getting tempo track", log: CCMIDIManager.midiLog, type: .error)
            return 0
        }
    }
    ...
    internal func sequenceData(musicSequence:MusicSequence, resolution:Int16=480) -> NSData? {
        ...
        let status = MusicSequenceFileCreateData(musicSequence,
                                                 ...
                                                 &data)
        if status != noErr {
            os_log("error turning MusicSequence into NSData %@", log: CCMIDIManager.midiLog, type: .error, status)
            checkError(status)
            return nil
        }
    ...
    func addTimeSignature(musicSequence:MusicSequence) {
        ...
        if let tempoTrack = tempo {
            ...
            if status != noErr {
                os_log("error adding time signature %@", log: CCMIDIManager.midiLog, type: .error, status)
                checkError(status)
            }
        }
    }
    ...
    internal func addCopyright(sequence:MusicSequence, text:String) {
        ...
        if let tempoTrack = tempo {
            status = MusicTrackNewMetaEvent(tempoTrack, 0, &metaEvent)
            if status != noErr {
                os_log("Unable to add copyright %@", log: CCMIDIManager.midiLog, type: .error, status)
            }
        }
    }
    ...
    internal func addLyric(track:MusicTrack, lyric:String, timeStamp:MusicTimeStamp) {
        ...
        if status != noErr {
            os_log("Unable to add lyric %@", log: CCMIDIManager.midiLog, type: .error, status)
            checkError(status)
        }
    }
    ...
    internal func addTrackName(track:MusicTrack, name:String) {
        ...
        if status != noErr {
            os_log("Unable to name Track %@", log: CCMIDIManager.midiLog, type: .error, status)
            checkError(status)
        }
    }
    ...
    internal func addSequenceName(sequence:MusicSequence, name:String) {
        ...
        if let tempoTrack = tempo {
            status = MusicTrackNewMetaEvent(tempoTrack, 0, &metaEvent)
            if status != noErr {
                os_log("borked %@", log: CCMIDIManager.midiLog, type: .error, status)
                checkError(status)
            }
        }
    ...
    internal func createMusicSequence() -> MusicSequence? {
        ...
        if let sequence = musicSequence {
            ...
            if status != noErr {
                os_log("error creating track %@", log: CCMIDIManager.midiLog, type: .error, status)
                checkError(status)
            }
            ...
            if let track = newtrack {
                ...
                if status != noErr {
                    os_log("error creating bank select event %@", log: CCMIDIManager.midiLog, type: .error, status)
                    checkError(status)
                }
                // bank select lsb
                ...
                status = MusicTrackNewMIDIChannelEvent(track, 0, &chanmess)
                if status != noErr {
                    os_log("error creating program change event %@", log: CCMIDIManager.midiLog, type: .error, status)
                    checkError(status)
                }
                ...
                for i:UInt8 in 60...72 {
                    ...
                    if status != noErr {
                        os_log("error creating new midi note event %@", log: CCMIDIManager.midiLog, type: .error, status)
                        checkError(status)
                    }
                    beat += 1
                }
                ...
                let sequencerCallback: MusicSequenceUserCallback =  {
                    ...
                    if userData.data == 0xAA {
                        os_log("got user event AA of length %d", log: CCMIDIManager.midiLog, type: .debug, userData.length)
                    }
                }
                ...
                os_log("sequence info %@", log: CCMIDIManager.midiLog, type: .debug, info as! CVarArg)
    ...
    internal func augraphSetup() {
        ...
        if let graph = self.processingGraph {
            ...
            os_log("core audio augraph is nil", log: CCMIDIManager.midiLog, type: .error)
        }
    }
    ...
    func getUniqueID(_ endpoint:MIDIEndpointRef) -> (OSStatus, MIDIUniqueID) {
        ...
        if status != noErr {
            os_log("error getting unique id. status %@", log: CCMIDIManager.midiLog, type: .error, status)
            checkError(status)
        }
        return (status,id)
    }
    ...
    func setUniqueID(_ endpoint:MIDIEndpointRef, id:MIDIUniqueID) -> OSStatus {
        ...
        if status != noErr {
            os_log("error setting unique id. status %@", log: CCMIDIManager.midiLog, type: .error, status)
    ...
    func getDeviceName(_ endpoint:MIDIEndpointRef) -> String? {
        ...
        if status != noErr {
            os_log("error getting device name for %@. status %@", log: CCMIDIManager.midiLog, type: .error, endpoint, status)
            checkError(status)
        }
    ...
    func allExternalDeviceProps() {
        ...
        os_log("external devices %d", log: CCMIDIManager.midiLog, type: .debug, n)
    ...
    func allDeviceProps() {
        ...
        os_log("number of devices %d", log: CCMIDIManager.midiLog, type: .debug, n)
    ...
    func allDestinationProps() {
        ...
        os_log("destinations %d", log: CCMIDIManager.midiLog, type: .debug, numberOfDestinations)
    ...
    func allSourceProps() {
        ...
        os_log("numberOfSources %d", log: CCMIDIManager.midiLog, type: .debug, numberOfSources)
    ...
    func printProperties(_ midiobject:MIDIObjectRef) {
        ...
        if let midiProperties: CFPropertyList = unmanagedProperties?.takeUnretainedValue() {
            if let midiDictionary = midiProperties as? Dictionary<String, Any> {
                os_log("MIDI properties %{public}@", log: CCMIDIManager.midiLog, type: .debug, midiDictionary)
                for (key, value) in midiDictionary {
                    print("key '\(key)', value '\(value)'")
                    // what a mess. and the public doesn't really help here.
                    os_log("key %{public}@ value %{public}@", log: CCMIDIManager.midiLog, type: .debug, key, value as! CVarArg)
                }

            }
        } else {
            os_log("Couldn't load properties for %@", log: CCMIDIManager.midiLog, type: .error, midiobject)
        }
    }
    ...
    func propertyValue(_ midiobject:MIDIObjectRef, propName:String) -> String? {
        ...
        if let midiProperties: CFPropertyList = unmanagedProperties?.takeUnretainedValue() {
            if let midiDictionary = midiProperties as? NSDictionary {
                os_log("MIDI properties %@", log: CCMIDIManager.midiLog, type: .debug, midiDictionary)
    ...
    internal func checkError(_ error:OSStatus) {
        ...
        if let s = CCMIDIManager.stringFrom4(status:error) {
            print("error string '\(s)'")
            os_log("error string %@", log: CCMIDIManager.midiLog, type: .error, s)
        }
        
        switch(error) {
            
        case kMIDIInvalidClient :
            os_log("kMIDIInvalidClient", log: CCMIDIManager.midiLog, type: .error)
            
        case kMIDIInvalidPort :
            os_log("kMIDIInvalidPort", log: CCMIDIManager.midiLog, type: .error)

        case kMIDIWrongEndpointType :
            os_log("kMIDIWrongEndpointType", log: CCMIDIManager.midiLog, type: .error)

        case kMIDINoConnection :
            os_log("kMIDINoConnection", log: CCMIDIManager.midiLog, type: .error)

        case kMIDIUnknownEndpoint :
            os_log("kMIDIUnknownEndpoint", log: CCMIDIManager.midiLog, type: .error)

        case kMIDIUnknownProperty :
            os_log("kMIDIUnknownProperty", log: CCMIDIManager.midiLog, type: .error)

        case kMIDIWrongPropertyType :
            os_log("kMIDIWrongPropertyType", log: CCMIDIManager.midiLog, type: .error)

        case kMIDINoCurrentSetup :
            os_log("kMIDINoCurrentSetup", log: CCMIDIManager.midiLog, type: .error)

        case kMIDIMessageSendErr :
            os_log("kMIDIMessageSendErr", log: CCMIDIManager.midiLog, type: .error)

        case kMIDIServerStartErr :
            os_log("kMIDIServerStartErr", log: CCMIDIManager.midiLog, type: .error)

        case kMIDISetupFormatErr :
            os_log("kMIDISetupFormatErr", log: CCMIDIManager.midiLog, type: .error)

        case kMIDIWrongThread :
            os_log("kMIDIWrongThread", log: CCMIDIManager.midiLog, type: .error)

        case kMIDIObjectNotFound :
            os_log("kMIDIObjectNotFound", log: CCMIDIManager.midiLog, type: .error)

        case kMIDIIDNotUnique :
            os_log("kMIDIIDNotUnique", log: CCMIDIManager.midiLog, type: .error)

        case kMIDINotPermitted :
            os_log("kMIDINotPermitted", log: CCMIDIManager.midiLog, type: .error)
            os_log("did you set UIBackgroundModes to audio in your info.plist?", log: CCMIDIManager.midiLog, type: .error)

        //AUGraph.h
        case kAUGraphErr_NodeNotFound:
            os_log("kAUGraphErr_NodeNotFound", log: CCMIDIManager.midiLog, type: .error)

        case kAUGraphErr_OutputNodeErr:
            os_log("kAUGraphErr_OutputNodeErr", log: CCMIDIManager.midiLog, type: .error)

        case kAUGraphErr_InvalidConnection:
            os_log("kAUGraphErr_InvalidConnection", log: CCMIDIManager.midiLog, type: .error)

        case kAUGraphErr_CannotDoInCurrentContext:
            os_log("kAUGraphErr_CannotDoInCurrentContext", log: CCMIDIManager.midiLog, type: .error)

        case kAUGraphErr_InvalidAudioUnit:
            os_log("kAUGraphErr_InvalidAudioUnit", log: CCMIDIManager.midiLog, type: .error)

            // core audio
            
        case kAudio_UnimplementedError:
            os_log("kAudio_UnimplementedError", log: CCMIDIManager.midiLog, type: .error)

        case kAudio_FileNotFoundError:
            os_log("kAudio_FileNotFoundError", log: CCMIDIManager.midiLog, type: .error)

        case kAudio_FilePermissionError:
            os_log("kAudio_FilePermissionError", log: CCMIDIManager.midiLog, type: .error)

        case kAudio_TooManyFilesOpenError:
            os_log("kAudio_TooManyFilesOpenError", log: CCMIDIManager.midiLog, type: .error)

        case kAudio_BadFilePathError:
            os_log("kAudio_BadFilePathError", log: CCMIDIManager.midiLog, type: .error)

        case kAudio_ParamError:
            os_log("kAudio_ParamError", log: CCMIDIManager.midiLog, type: .error)

        case kAudio_MemFullError:
            os_log("kAudio_MemFullError", log: CCMIDIManager.midiLog, type: .error)
        ...
        case kAudioToolboxErr_InvalidSequenceType :
            os_log("kAudioToolboxErr_InvalidSequenceType", log: CCMIDIManager.midiLog, type: .error)

        case kAudioToolboxErr_TrackIndexError :
            os_log("kAudioToolboxErr_TrackIndexError", log: CCMIDIManager.midiLog, type: .error)

        case kAudioToolboxErr_TrackNotFound :
            os_log("kAudioToolboxErr_TrackNotFound", log: CCMIDIManager.midiLog, type: .error)

        case kAudioToolboxErr_EndOfTrack :
            os_log("kAudioToolboxErr_EndOfTrack", log: CCMIDIManager.midiLog, type: .error)

        case kAudioToolboxErr_StartOfTrack :
            os_log("kAudioToolboxErr_StartOfTrack", log: CCMIDIManager.midiLog, type: .error)

        case kAudioToolboxErr_IllegalTrackDestination :
            os_log("kAudioToolboxErr_IllegalTrackDestination", log: CCMIDIManager.midiLog, type: .error)

        case kAudioToolboxErr_NoSequence :
            os_log("kAudioToolboxErr_NoSequence", log: CCMIDIManager.midiLog, type: .error)

        case kAudioToolboxErr_InvalidEventType :
            os_log("kAudioToolboxErr_InvalidEventType", log: CCMIDIManager.midiLog, type: .error)

        case kAudioToolboxErr_InvalidPlayerState :
            os_log("kAudioToolboxErr_InvalidPlayerState", log: CCMIDIManager.midiLog, type: .error)

            // AudioUnit
            
        case kAudioUnitErr_InvalidProperty :
            os_log("kAudioUnitErr_InvalidProperty", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_InvalidParameter :
            os_log("kAudioUnitErr_InvalidParameter", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_InvalidElement :
            os_log("kAudioUnitErr_InvalidElement", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_NoConnection :
            os_log("kAudioUnitErr_NoConnection", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_FailedInitialization :
            os_log("kAudioUnitErr_FailedInitialization", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_TooManyFramesToProcess :
            os_log("kAudioUnitErr_TooManyFramesToProcess", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_InvalidFile :
            os_log("kAudioUnitErr_InvalidFile", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_FormatNotSupported :
            os_log("kAudioUnitErr_FormatNotSupported", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_Uninitialized :
            os_log("kAudioUnitErr_Uninitialized", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_InvalidScope :
            os_log("kAudioUnitErr_InvalidScope", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_PropertyNotWritable :
            os_log("kAudioUnitErr_PropertyNotWritable", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_InvalidPropertyValue :
            os_log("kAudioUnitErr_InvalidPropertyValue", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_PropertyNotInUse :
            os_log("kAudioUnitErr_PropertyNotInUse", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_Initialized :
            os_log("kAudioUnitErr_Initialized", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_InvalidOfflineRender :
            os_log("kAudioUnitErr_InvalidOfflineRender", log: CCMIDIManager.midiLog, type: .error)

        case kAudioUnitErr_Unauthorized :
            os_log("kAudioUnitErr_Unauthorized", log: CCMIDIManager.midiLog, type: .error)

        default:
            os_log("huh?", log: CCMIDIManager.midiLog, type: .error)
